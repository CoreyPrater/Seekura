...
$gatewayFile = "$appDir\server\gateway.ts"   # the new single-URL gateway
$ltToken     = $env:Terick1
...
Start-Process -FilePath "cmd.exe" `
    -ArgumentList "/c deno run --allow-net --allow-read --allow-env $ollamaDir\ollamaProxy.js" `
...
Start-Process -FilePath "cmd.exe" `
    -ArgumentList "/c deno run --allow-net --allow-read --allow-env $gatewayFile" `
...
Start-Process -FilePath "cmd.exe" `
    -ArgumentList "/c lt --port 8000 --subdomain $subdomain --token $ltToken" `
    -WorkingDirectory $appDir
