/*
  ollamaProxy.ts — Seekura full-featured proxy (session + history)
  Updated for Deno 0.177.0, using FormData for uploads
*/
...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "POST" && /^\/characters\/\d+\/upload$/.test(path)) {
      ...
      await Deno.writeFile(`${ASSETS_DIR}${filename}`, fileData);
    ...
    if (req.method === "POST" && path === "/chat") {
      ...
      try {
        // Use generateCharacterReply from chatHandler.ts
